...
- **개발 엔진 및 언어**: Unity & C#
- **팀 구성**: TEAM4 (강현서, 박선준, 양경덕, Nitu Cristina)
...
- **주요 특징**:
  - **AR 기반 실감 전투**: Unity XR/AR Kit를 활용하여, 실제 주변 환경을 게임 배경으로 사용합니다. 플레이어는 현실 세계에서 직접 전투를 수행하는 듯한 몰입감을 경험할 수 있습니다.
  - **스테이지 기반 진행**: 게임은 여러 스테이지로 나뉘며, 각 스테이지는 특정 목표를 달성하거나 보스를 처치하는 방식으로 진행됩니다. 레벨이 올라갈수록 도전적인 난이도를 제공합니다.
  - **몬스터 처치**: 기본 몬스터들은 스테이지 초반에 등장하여 플레이어를 공격하며, 이들을 모두 처치한 후 보스가 등장합니다. 보스 몬스터는 고유의 강력한 공격 패턴을 가지며, 플레이어는 이를 전략적으로 대응해야 합니다.
...
- **사용 방법**: 플레이어가 이미지를 카메라에 인식시키면, 해당 이미지 위에 특정 파티클이 생성됩니다. 이 파티클을 클릭하면 스킬 버튼이 활성화되며, 스킬을 사용할 수 있습니다.
